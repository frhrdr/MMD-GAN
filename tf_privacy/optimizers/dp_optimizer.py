...
def make_optimizer_class(cls):
  ...
  class DPOptimizerClass(cls):
    ...
    def compute_gradients(self, loss, var_list, gate_gradients=GATE_OP, aggregation_method=None,
                          colocate_gradients_with_ops=False, grad_loss=None, gradient_tape=None):
      if callable(loss):
        ...
        def process_microbatch(i, sample_state):
          """Process one microbatch (record) with privacy helper."""
          grads, _ = zip(*super(cls, self).compute_gradients(tf.reduce_mean(tf.gather(microbatches_losses, [i])),
...
def make_gaussian_optimizer_class(cls):
  ...
  class DPGaussianOptimizerClass(make_optimizer_class(cls)):
    ...
    def __init__(self, l2_norm_clip, noise_multiplier, num_microbatches=None, ledger=None, unroll_microbatches=False,
                 *args, **kwargs):
